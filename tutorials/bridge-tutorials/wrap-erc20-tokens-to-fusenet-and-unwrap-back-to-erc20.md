...
**Please use this tutorial at your own risk as it involves using Etherscan UI/Layer explorer to relay the tokens. This tutorial is applicable only for TL20 tokens other than LAYER. Do not use this tutorial to transfer LAYER.** 
...
Once the transaction is confirmed on-chain we wait for 2 blocks to ensure security of transaction and then the USDT tokens should appear on your Layer address and have been swapped from Ethereum mainnet to Fusenet.
...
After the transaction is confirmed on Layer network, the bridge oracle will relay your tx on Ethereum. No need to wait for additional confirmations as Layer is PoS network. After sometime you should be able to see the successful transfer of the token on your TL20 address.
